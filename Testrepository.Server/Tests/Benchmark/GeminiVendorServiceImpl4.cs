...
public class GeminiVendorServiceImpl4 : IGptVendorService
{
    private readonly IGenerativeModel _geminiModel;

    public string VendorName => "Gemini 2.0 Flash Lite"; // Displayed in your benchmark UI

    public GeminiVendorServiceImpl4(IConfiguration config)
    {
        var apiKey = config["Gemini:ApiKey"] ?? throw new ArgumentException("Gemini:ApiKey missing");
        var googleAI = new GoogleAi(apiKey);
        _geminiModel = googleAI.CreateGenerativeModel("models/gemini-2.0-flash-lite");
    }
